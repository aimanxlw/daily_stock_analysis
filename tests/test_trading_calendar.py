...
class CalendarCoverageDegradationTestCase(unittest.TestCase):
    """日历未覆盖该日期时，必须降级为「仅排除周末」并给出告警。

    背景：exchange-calendars 的 XSHG 日历只硬编码到 2026 年底，AkShare 的交易日历
    同样截止 2026-12-31（中国节假日安排由官方逐年公布，无法预知更远）。原实现
    在超范围时 fail-open 返回 True，等于把周末和节假日都当成交易日推送。
    """

    class _BoundedCalendar:
        """最小日历替身：固定覆盖区间 + 显式交易日集合。"""

        def __init__(self, first: str, last: str, sessions=()):
            self.first_session = pd.Timestamp(first)
            self.last_session = pd.Timestamp(last)
            self._sessions = {pd.Timestamp(s).date() for s in sessions}

        def is_session(self, session):
            return pd.Timestamp(session).date() in self._sessions

    def setUp(self):
        trading_calendar._DEGRADED_CALENDAR_WARNED.clear()

    def _patched(self, cal):
        return (
            patch.object(trading_calendar, "_XCALS_AVAILABLE", True),
            patch.object(trading_calendar.xcals, "get_calendar", return_value=cal),
        )

    def test_out_of_range_weekend_is_not_trading_day(self):
        cal = self._BoundedCalendar("2020-01-01", "2026-12-31")
        p1, p2 = self._patched(cal)
        with p1, p2:
            # 2027-01-03 是周日：降级判断必须排除周末
            self.assertFalse(trading_calendar.is_market_open("cn", date(2027, 1, 3)))

    def test_out_of_range_weekday_is_trading_day_and_warns(self):
        cal = self._BoundedCalendar("2020-01-01", "2026-12-31")
        p1, p2 = self._patched(cal)
        with p1, p2, self.assertLogs(
            "src.core.trading_calendar", level="WARNING"
        ) as logs:
            # 2027-01-04 是周一：降级后按工作日放行，但必须留下告警
            self.assertTrue(trading_calendar.is_market_open("cn", date(2027, 1, 4)))
        self.assertTrue(any("已降级" in line for line in logs.output))

    def test_degradation_warning_deduplicated_per_market_and_year(self):
        cal = self._BoundedCalendar("2020-01-01", "2026-12-31")
        p1, p2 = self._patched(cal)
        with p1, p2, self.assertLogs(
            "src.core.trading_calendar", level="WARNING"
        ) as logs:
            trading_calendar.is_market_open("cn", date(2027, 1, 4))
            trading_calendar.is_market_open("cn", date(2027, 1, 5))
        degraded = [line for line in logs.output if "已降级" in line]
        self.assertEqual(len(degraded), 1)

    def test_in_range_dates_still_use_exact_calendar(self):
        cal = self._BoundedCalendar(
            "2020-01-01", "2026-12-31", sessions=["2026-09-24"]
        )
        p1, p2 = self._patched(cal)
        with p1, p2:
            self.assertTrue(trading_calendar.is_market_open("cn", date(2026, 9, 24)))
            # 覆盖范围内但不在交易日集合中（如中秋）-> 精确判为休市，不降级
            self.assertFalse(trading_calendar.is_market_open("cn", date(2026, 9, 25)))

    def test_calendar_unavailable_still_excludes_weekend(self):
        with patch.object(trading_calendar, "_XCALS_AVAILABLE", False):
            self.assertFalse(trading_calendar.is_market_open("cn", date(2027, 1, 3)))
            self.assertTrue(trading_calendar.is_market_open("cn", date(2027, 1, 4)))

    def test_unknown_market_stays_fail_open(self):
        with patch.object(trading_calendar, "_XCALS_AVAILABLE", True):
            # 没有对应日历的市场不能擅自判为休市
            self.assertTrue(trading_calendar.is_market_open("xx", date(2027, 1, 3)))

    def test_get_calendar_coverage_reports_windows(self):
        coverage = trading_calendar.get_calendar_coverage()
        self.assertIn("cn", coverage)
